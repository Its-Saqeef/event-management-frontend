...
import { useMyEvents } from "@/services/event-service";
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <Tabs defaultValue="tickets" className="space-y-8">
          <TabsList className="bg-white/5 border border-white/10 p-1 h-auto rounded-xl">
            <TabsTrigger value="tickets" className="rounded-lg px-6 py-2 text-base data-[state=active]:bg-primary data-[state=active]:text-white">{user?.role === "organizer" ? "My Events" : "My Tickets"}</TabsTrigger>
          ...
          <TabsContent value="tickets" className="space-y-4">
            {isLoading ? (
              ...
              <div className="text-center py-12 border border-dashed border-white/10 rounded-3xl bg-white/5">
                ...
                <Button variant="link" className="text-primary mt-2">Explore Events</Button>
              </div>
            ) : (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {events.map((event) => (
                  <div key={event._id} className="relative group cursor-pointer" onClick={() => setLocation(`/event/${event.slug}`)}>
                 ...
                 <div key={i} className="flex gap-4 p-4 rounded-xl bg-white/5 border border-white/10 items-start">
                   ...
                   <div>
                     ...
                     <p className="text-sm text-muted-foreground mt-1">
                       {events && events.length > 0 
                         ? `Don't forget! ${events[0].title} is coming up in 2 days. Get ready for an amazing experience.`
